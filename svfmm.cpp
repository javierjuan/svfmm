...
std::string doc()
{
    ...
    ss << "USAGE:" << std::endl;
    ss << "\tsvfmm(input, k, <option>=<value>)" << std::endl;
    ...
    try
    {
        if (tropism == "isotropic")
        {
            if (estimation == "frequentist")
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new FrequentistIsotropicGaussianMarkovRandomField<Dimensions>(mask, k, topology));
            else
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new BayesianIsotropicGaussianMarkovRandomField<Dimensions>(mask, k, topology));
        }
        else
        {
            if (estimation == "frequentist")
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new FrequentistAnisotropicGaussianMarkovRandomField<Dimensions>(mask, k, topology));
            else
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new BayesianAnisotropicGaussianMarkovRandomField<Dimensions>(mask, k, topology));
        }
