...
/*                                                                         */
/***************************************************************************/
/* Non Local Spatially Variant Finite Mixture Models                       */
/***************************************************************************/
...
std::string doc()
{
    ...
    ss << "Non-Local Spatially Varying Finite Mixture Models" << std::endl;
    ss << "-------------------------------------------------" << std::endl;

    ss << "USAGE:" << std::endl;
    ss << "\tnlsvfmm(input, k, <option>=<value>)" << std::endl;

    ss << "COMMAND:" << std::endl;
    ss << "\tnlsvfmm:" << std::endl;
    ss << "\t\tA spatially varying finite mixture model segmentation algorithm that introduces a continuous Non-Local Markov Random Field (MRF) over the prior probabilities of the model to impose spatial constraints during the learning process." << std::endl;
    ss << "OPTIONS:" << std::endl;
    ...
    ss << "\tpatch=True/False" << std::endl;
    ss << "\t\tPatch-based MRF. User can indicate if the Non-local MRF similarities should be computed voxel-wise or patch-wise." << std::endl;
...
template<typename Distribution, int Dimensions, bool PatchBased>
    ...
    try
    {
        if (tropism == "isotropic")
        {
            if (estimation == "frequentist")
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new FrequentistIsotropicNonLocalMarkovRandomField<Dimensions, PatchBased>(mask, k, topology));
            else
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new BayesianIsotropicNonLocalMarkovRandomField<Dimensions, PatchBased>(mask, k, topology));
        }
        else
        {
            if (estimation == "frequentist")
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new FrequentistAnisotropicNonLocalMarkovRandomField<Dimensions, PatchBased>(mask, k, topology));
            else
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new BayesianAnisotropicNonLocalMarkovRandomField<Dimensions, PatchBased>(mask, k, topology));
        }
             ...
             const std::string &tropism, const std::string &estimation, const bool patch, const int max_iterations, const double regularization, const double threshold, const bool verbose)
{
    ...
    if (verbose)
    {
        ...
        std::cout << "\tMRF Patch-based: " << (patch ? "True" : "False") << std::endl;
    ...
    if (dimensions == 3)
    {
        if (patch)
        {
            if (distribution == "gaussian")
                return classify<MultivariateNormal, 3, true>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else if (distribution == "student")
                return classify<MultivariateTStudent, 3, true>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else
                return classify<Gamma, 3, true>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
        }
        else
        {
            if (distribution == "gaussian")
                return classify<MultivariateNormal, 3, false>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else if (distribution == "student")
                return classify<MultivariateTStudent, 3, false>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else
                return classify<Gamma, 3, false>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
        }
    }
    else
    {
        ...
        {
            if (distribution == "gaussian")
                return classify<MultivariateNormal, 2, true>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else if (distribution == "student")
                return classify<MultivariateTStudent, 2, true>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else
                return classify<Gamma, 2, true>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
        }
        else
        {
            if (distribution == "gaussian")
                return classify<MultivariateNormal, 2, false>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else if (distribution == "student")
                return classify<MultivariateTStudent, 2, false>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
            else
                return classify<Gamma, 2, false>(image, mask_, k, topology_, tropism, estimation, labels, max_iterations, threshold, regularization, verbose);
        }
    }
...
PYBIND11_MODULE(nlsvfmm, m)
{
    ...
    m.def("fit", &fit,
        ...
        py::arg("estimation") = estimation, py::arg("patch") = patch, 
        py::arg("max_iterations") = max_iterations, py::arg("regularization") = regularization,
        py::arg("threshold") = threshold, py::arg("verbose") = verbose
    );
}
