...
/*                                                                         */
/***************************************************************************/
/* t-Student Spatially Variant Finite Mixture Models                       */
/***************************************************************************/
...
std::string doc()
{
    ...
    ss << "t-Student Spatially Varying Finite Mixture Models" << std::endl;
    ss << "-------------------------------------------------" << std::endl;

    ss << "USAGE:" << std::endl;
    ss << "\ttstsvfmm(input, k, <option>=<value>)" << std::endl;

    ss << "COMMAND:" << std::endl;
    ss << "\ttstsvfmm:" << std::endl;
    ss << "\t\tA spatially varying finite mixture model segmentation algorithm that introduces a continuous t-Student Markov Random Field (MRF) over the prior probabilities of the model to impose spatial constraints during the learning process." << std::endl;
    ss << "OPTIONS:" << std::endl;
    ...
    try
    {
        if (tropism == "isotropic")
        {
            if (estimation == "frequentist")
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new FrequentistIsotropicTStudentMarkovRandomField<Dimensions>(mask, k, topology));
            else
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new BayesianIsotropicTStudentMarkovRandomField<Dimensions>(mask, k, topology));
        }
        else
        {
            if (estimation == "frequentist")
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new FrequentistAnisotropicTStudentMarkovRandomField<Dimensions>(mask, k, topology));
            else
                spatialCoefficients = std::unique_ptr<MarkovRandomField<Dimensions>>(new BayesianAnisotropicTStudentMarkovRandomField<Dimensions>(mask, k, topology));
        }
